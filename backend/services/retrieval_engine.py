...
class RetrievalEngine:
    ...
    def retrieve(self, query: str, top_k: int = 10) -> List[ScoredChunk]:
        """
        Retrieve relevant chunks for query with relaxed dynamic K-cutoff.

        Implements the following filtering strategy:
        1. Embed the user query
        2. Perform similarity search in vector store (top_k=10)
        3. Apply relevance threshold (score > 0.15) to filter junk matches
        4. Apply relaxed dynamic K-cutoff: only include chunks within 50% of top score
           - Example: If top chunk scores 0.80, include chunks >= 0.40
           - Keeps more context so the LLM has sufficient material to answer
        ...
        try:
            ...
            # Step 3: Apply relevance threshold — low bar, just filter noise
            ...
            # Step 4: Apply relaxed dynamic K-cutoff (within 50% of top score)
            top_score = filtered_chunks[0].relevance_score
            cutoff_threshold = top_score * 0.5  # 50% of top score — much more permissive
